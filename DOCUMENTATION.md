...
- Apart from conflict stalls, forwarding adds **zero latency** (same
  ...
  hierarchy (verified: `stride1` matched to the tick).
...
Structural — `m5out/config.ini`: core's `dcache_port` peers with
`...splitter.cpu_side_port`; `scalar_side_port`/`vector_side_port` peer
with the two L1Ds; both L2s' `mem_side` on the membus. `config.dot.svg`
shows the parallel diamond.

Steering — `m5out/stats.txt`:
- `splitters.vectorReqs` + `splitters.scalarReqs` ≈ baseline L1D demand
  accesses; `splitters.untaggedReqs` == 0.
- Pure-scalar binary: `vector-l1d-cache-0` demand accesses exactly 0, and
  total cycles identical to the same run without `--vector-cache`.
- `--debug-flags=VectorSplitter` prints one steering line per request,
  composing with the `Packet::print` extension tag:
  `Steering ReadReq [27e10:27e17] type=SimdUnitStrideLoad to vector side`.

Correctness — `rvv_load_chain_test.bin` (vectorized-newlib binary that
breaks under `--scalar-uncacheable`) must run clean with `--vector-cache`;
`check_ara_latencies.py` and `run_load_chain_test.py` must still pass
(chaining timing flows through `completeAcc` off the dcache response path).
